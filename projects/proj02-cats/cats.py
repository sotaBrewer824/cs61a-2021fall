...
def choose(paragraphs, select, k):
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    paragraph_list = [x for x in paragraphs if select(x)]
    return paragraph_list[k] if k < len(paragraph_list) else ""
    # END PROBLEM 1
...
def about(topic):
    ...
    def about_helper(paragraph):
        preprcessed_paragraph = split(lower(remove_punctuation(paragraph)))
        return any([t in preprcessed_paragraph for t in topic])
    return about_helper
...
def accuracy(typed, reference):
    ...
    for i in range(min_length):
        if typed_words[i] == reference_words[i]:
            correct_count += 1
    score = correct_count / len(typed_words) * 100.0
    return score
    # END PROBLEM 3
...
def report_progress(sofar, prompt, user_id, upload):
    ...
    for i in range(len(sofar)):
        if sofar[i] == prompt[i]:
            ...
            break
    progress_ratio = correct_count / len(prompt)
    p = {'id': user_id, 'progress': progress_ratio}
    upload(p)
...
def time_per_word(words, times_per_player):
    ...
    """
    # BEGIN PROBLEM 9
    times = [[x[i + 1]-x[i] for i in range(len(x) - 1)] \
